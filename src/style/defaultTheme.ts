...
import { GUThemeCSSFunc } from './util'
...
const defaultColorTheme: GUColorTheme = {
  secondary: {
    dark: '#077C7D',
    main: '#00B7B8',
    light: '#6AE1E2',
...
const inputTextAreaBase: GUThemeCSSFunc = ({ color, radius }) => css`
  /* Wrapper rules */
  display: flex;
...
const radioCheckboxBase: GUThemeCSSFunc = ({ color, radius }) => css`
  display: flex;
  align-items: center;
  margin: 5px 0;
...
export const guDefaultTheme = {
  ...
  button: {
    css: ({ color, radius }, p?: GUCommonStyleProps) => {
